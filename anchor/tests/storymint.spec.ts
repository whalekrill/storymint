...
import {
  ...
  mintAsset,
  updateMetadata,
} from '../../src/clients/localnet/generated/umi/src/instructions'
...
describe('Storymint', () => {
  ...
  it('should update metadata URI and name', async () => {
    ...
    const newName = 'Storymint Updated'
    const newUri = 'https://storymint.web.app/metadata/updated.json'
  ...
  it('should fail with unauthorized authority', async () => {
    ...
    try {
      await updateMetadata(umi, {
        ...
        payer,
        args: {
          name: 'Storymint Updated',
          uri: 'https://storymint.web.app/metadata/updated.json',
        },
      }).sendAndConfirm(umi)
      fail('Should have thrown error')
    } catch (error) {
